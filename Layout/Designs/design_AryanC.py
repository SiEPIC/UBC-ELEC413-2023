...
def design_AryanC(cell, cell_y, inst_wg1, inst_wg2, inst_wg3, waveguide_type):
    ...
    inst_y1 = connect_cell(inst_wg1, 'opt2', cell_y, 'opt2') # opt1<---inst_wg1--->opt2 + opt2<---cell_y--->opt1 = opt1<---inst_y1--->opt1 ????If Inst_y1 has 2 opt1 pins, then how do we know which one to connect to????????
    inst_taper1 = connect_cell(inst_y1, 'opt1', cell_taper, 'pin1') # opt1<---inst_y1--->opt1 + pin1<---cell_taper--->pin2 = opt1<---inst_taper1--->pin2
    inst_bragg1 = connect_cell(inst_taper1, 'pin2', cell_bragg, 'opt1') # opt1<---inst_taper1--->pin2 + opt1<---cell_bragg--->opt2 = opt1<---inst_bragg1--->opt2
    inst_bragg2 = connect_cell(inst_bragg1, 'opt2', cell_bragg, 'opt2') # opt1<---inst_bragg1--->opt2 + opt2<---cell_bragg--->opt1 = opt1<---inst_bragg2--->opt1  ????If Bragg2 has 2 opt1 pins, then how do we know which one to connect to????????
    ...
    inst_taper2 = connect_cell(inst_bragg2, 'opt1', cell_taper, 'pin2') # opt1<---inst_bragg2--->opt1 + pin2<---cell_taper--->pin1 = opt1<---inst_taper2--->pin1
    connect_pins_with_waveguide(inst_taper2, 'opt1', inst_wg2, 'opt1', waveguide_type=waveguide_type) # in case of further errors please change opt1 to pin1 :(
    '''
